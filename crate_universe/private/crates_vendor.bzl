...
def _write_splicing_manifest(ctx):
    ...
    manifest = write_splicing_manifest(
        ctx,
        write_data_file = _write_data_file,
...
def write_splicing_manifest(ctx, write_data_file, packages, splicing_config, cargo_config, manifests, manifest_to_path):
...
def _write_config_file(ctx):
    config = write_config_file(
        ctx,
        regen_command = "bazel run {}".format(ctx.label),
        mode = ctx.attr.mode,
...
def write_config_file(ctx, regen_command, mode, annotations, generate_build_scripts, supported_platform_triples, repository_name, output_pkg, workspace_name, write_data_file):
    rendering_config = dict(json.decode(render_config(regen_command = regen_command)))
